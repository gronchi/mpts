...
% The program is updated for reading isf files saved in ascii format: 
% [time(s), signal(V)]
...
for s=1:numel(fileNames)
    ...
    % MK start ======
        ...
        while not(feof(fileID))
           Signal=[Signal;str2num([c,fgetl(fileID)])];
           c=[]; 
        end; 
        count=length(Signal);
        yBytes=2*count; 
        header.NR_PT=count;
        header.PT_OFF=find(Signal(:,1)>0,1,'first')-1;
        header.XINCR= Signal(3,1)-Signal(2,1);
        binaryData=Signal(:,2);
    ...
    % Check that there is no leftover data. I found that there generally is.
    if( ~feof(fileID) )
